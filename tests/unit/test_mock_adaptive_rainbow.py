"""
Modified test file for adaptive rainbow agent that works with both PyTorch and mock implementations.
"""
...
from qtrust.implementation_switch import get_adaptive_rainbow_agent, set_use_pytorch

# Set to use mock implementation for testing
set_use_pytorch(False)

class TestMockAdaptiveRainbowAgent(unittest.TestCase):
    """Test cases for the mock Adaptive Rainbow DQN agent."""
    
    def setUp(self):
        """Set up test fixtures."""
        self.state_dim = 10
        self.action_dim = 4
        self.agent = get_adaptive_rainbow_agent(self.state_dim, self.action_dim)
    ...
    def test_adaptation(self):
        ...
        # Force adaptation by updating many times
        for _ in range(100):
            state = np.random.rand(self.state_dim)
            action = self.agent.select_action(state)
            reward = np.random.rand()
        ...
        self.assertTrue(hasattr(self.agent, 'adaptation_counter'))
        
    def test_save_load(self):
        """Test save and load functionality."""
        # This is just a mock test since we're not actually saving/loading
        self.agent.save("test_adaptive_model.pt")
        self.agent.load("test_adaptive_model.pt")
